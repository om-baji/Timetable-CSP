...
export class TimeTableCSP {
    ...
    private isAvailable(day: number, slot: number, faculty: number, batch: number, room: number, isConsecutive: boolean = false): boolean {
        if (slot === LUNCH_SLOT) return false;

        if (faculty !== -1 && this.facultyBusy[faculty][day][slot]) return false;

        if (batch !== -1) {
            if (this.batchBusy[batch][day][slot]) return false;
        } else {
            for (let b = 0; b < BATCHES; b++) {
                if (this.batchBusy[b][day][slot]) return false;
            }
        }

        if (this.roomBusy[day * TIME_SLOTS + slot][room]) return false;
